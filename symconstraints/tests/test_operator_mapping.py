...
def test_validate_dict():
    ...
    validation = Validation(
        frozenset([a, b]),
        frozenset([validation_op])
    )

    valid_result = validate_mapping(validation, {
        'a': 5,
        'b': 8
    })
    assert valid_result
    ...
    validation_result_match = re.fullmatch(r'Mapping (\{[^}]*\}) is invalid due to not satisfying ' + re.escape(f'[{validation_op}]'), str(invalid_result))
...
def test_validate_constraint():
    ...
    assert not invalid_result
    unsatisfied_validations = [validation_result for validation_result in invalid_result.validation_results if len(validation_result.unsatisfied_booleans) > 0]
    assert len(unsatisfied_validations) == 1
    assert unsatisfied_validations[0].unsatisfied_booleans == [a<c]

    assert str(invalid_result) == f'Mapping is invalid due to:\n- {unsatisfied_validations[0]}'
